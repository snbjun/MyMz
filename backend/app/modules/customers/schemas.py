...
class DecimalStringMixin(BaseModel):
    @field_serializer("opening_receivable", "current_receivable", "credit_limit", check_fields=False)
...
class CustomerRead(DecimalStringMixin):
    id: int
    code: str | None = None
    name: str
    ...
    def fill_category_name(cls, value):
        if hasattr(value, "category"):
            data = {
                ...
                "opening_receivable": value.opening_receivable,
                "current_receivable": value.current_receivable,
...
class CustomerBase(DecimalStringMixin):
    ...
    opening_receivable: Decimal = Field(default=Decimal("0.00"), ge=Decimal("0.00"))
    current_receivable: Decimal | None = Field(default=None, ge=Decimal("0.00"))
...
class CustomerUpdate(BaseModel):
    ...
    address: str | None = Field(default=None, max_length=255)
    tax_number: str | None = Field(default=None, max_length=64)
    opening_receivable: Decimal | None = Field(default=None, ge=Decimal("0.00"))
    current_receivable: Decimal | None = Field(default=None, ge=Decimal("0.00"))
