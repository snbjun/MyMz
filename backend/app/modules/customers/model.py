...
class CustomerCategory(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_customer_categories_deleted_at", "deleted_at"),
    )


class Customer(Base):
    """第 3 阶段新增：客户资料，金额字段统一使用 Numeric/Decimal。"""
    ...
    category_id: Mapped[int | None] = mapped_column(ForeignKey("customer_categories.id"))
    ...
    opening_receivable: Mapped[Decimal] = mapped_column(
        Numeric(18, 2),
        nullable=False,
        default=Decimal("0.00"),
    )
    current_receivable: Mapped[Decimal] = mapped_column(
        Numeric(18, 2),
        nullable=False,
        default=Decimal("0.00"),
    )
    ...
    category: Mapped[CustomerCategory | None] = relationship(back_populates="customers")
    __table_args__ = (
        ...
        Index("ix_customers_category_id", "category_id"),
