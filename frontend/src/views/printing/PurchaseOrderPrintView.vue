...
async function fetchData() {
  ...
  try {
    ...
    const response = await getPurchaseOrderPrintData(id);
...
async function saveSettings(payload: PrintSettingPayload) {
  await updatePrintSetting("purchase_order", payload);
  ...
  <section class="print-page" v-loading="loading">
    ...
    <article v-if="data" class="print-sheet">
      ...
      <header class="print-header">
        ...
        <h1>{{ t("purchaseOrderPrintTitle") }}</h1>
        <p v-if="settings?.show_contact && settings.contact_text" class="contact-text">{{ settings.contact_text }}</p>
      </header>

      <div class="print-meta">
        <span>{{ t("purchaseOrderNo") }}：{{ data.order_no }}</span>
        <span>{{ t("purchaseDate") }}：{{ data.order_date }}</span>
        <span>{{ t("purchaseStatus") }}：{{ statusText(data.status) }}</span>
        <span>{{ t("supplierName") }}：{{ data.supplier_name }}</span>
        <span>{{ t("phone") }}：{{ data.supplier_phone || "-" }}</span>
        <span>{{ t("address") }}：{{ data.supplier_address || "-" }}</span>
      ...
      <div class="print-summary">
        ...
        <span>{{ t("payableAmount") }}：{{ money(data.payable_amount) }}</span>
